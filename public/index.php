...
class SU{
	...
	*/
	public static function createWebApplication($confPath=''){
		...
		!defined('__APP_NAMEDIR__')   && define('__APP_NAMEDIR__',self::getBaseDir($confPath));
	...
	*/
	private static function initialize($config){
		try{
			...
			self::setCookies(true);
			self::setView(true);
			self::setSecurity(true);
			self::setSystemAlias(true);
			self::setEventsManager(true);
			self::setCache(@$config->components->cache);
			self::setNamespaces(@$config->application->namespace);
			self::setBaseUri(@$config->application->baseUri);
			self::setRouter(@$config->components->router);
			self::setCrypt(@$config->components->crypt);
			self::setVolt(@$config->components->volt);
			self::setDataBase(@$config->components->database);
			self::setMetadata(@$config->components->metadata);
			self::setDispatcher(true);
			#~~~~~~~~~~~~~~
	...
	* @
	*/
	private static function getBaseDir($confPath){
		$path=str_replace(str_replace('\\','/',$_SERVER['DOCUMENT_ROOT']).'/','',str_replace('\\','/',realpath($confPath)));
		$paths=explode('/',$path);
		return $paths[0];
	}
	/*******************
	...
	* @
	*/
	private static function setDI(&$di=null){
	...
	* @
	*/
	private static function setModules(&$application){
		$modules  = @self::getDI()->get('config')->modules->toArray();
		if(!$modules){return;}
	...
	* @
	*/
	private static function setNamespaces($config){
		if(!$config){return;}
		$di=self::getDI();
		$namespace=array();
	...
	* @
	*/
	private static function setBaseUri($baseUri){
		!$baseUri && $baseUri='/';
		self::getDI()->set('url', function() use($baseUri){
	...
	* @
	*/
	private static function setRouter($router){
		$di=self::getDI();
	...
	* @
	*/
	private static function setSession($session){
		if(!$session){return;}
		self::getDI()->set('session', function() {
	...
	* @
	*/
	private static function setCookies($cookies){
		if(!$cookies){return;}
		self::getDI()->set('cookies', function () {
	...
	* @
	*/
	private static function setCrypt($crypt){
		if(!$crypt){return;}
		self::getDI()->set('crypt', function() use ($crypt) {
	...
	* @
	*/
	private static function setSecurity($security){
		if(!$security){return;}
		self::getDI()->set('security', function(){
	...
	* @
	*/
	private static function setVolt($volt){
		if(!$volt){return;}
		$di=self::getDI();
	...
	* @
	*/
	private static function setDataBase($database){
		if(!$database){return;}
		$di=self::getDI();
	...
	* @
	*/
	private static function setMetadata($modelsMetadata){
	...
	* @
	*/
	private static function setEventsManager($isTrue){
		if(!$isTrue){return;}
		self::getDI()->set('eventsManager',function(){
	...
	* @	
	*/
	private static function setView($isTrue){
		if(!$isTrue){return;}
		$config=array('.html' => 'volt');
		self::getDI()->set('view', function() use ($config){
			$view = new \Phalcon\Mvc\View();
			$view->registerEngines($config);
			return $view;
		});
	}
	/*******************
	...
	* @
	*/
	private static function setDispatcher($isTrue){
		if(!$isTrue){return;}
		$di=self::getDI();
	...
	* @
	*/
	final static function evalError($message=''){
		$di=self::getDI();
		$errorHandler=@$di->get('config')->components->errorHandler;
		$debug=@$di->get('config')->application->debug;
		if(!$errorHandler){
			$message=$message!=''?$message:'__ERROR__::is null message__';
			$debug && exit($message);
		}
		...
		$current_error_class=$dispatcher->getDefaultNamespace().'\\'.ucfirst($forward['controller'].'Controller');
		if(class_exists($current_error_class)){
			if(method_exists($current_error_class,$forward['action'].'Action')){
				$dispatcher->forward($forward);
			}else{
				$debug && exit('__CURRENT_ERROR__::'.str_replace('\\','.',$current_error_class).'__'.$forward['action'].'Action__');
			}
		}else{
			$base_error_class=__APP_BASESPACE__.'\\Controller\\'.ucfirst($forward['controller'].'Controller');
			if(method_exists($base_error_class,$forward['action'].'Action')){
				call_user_func_array(array(new $base_error_class,$forward['action'].'Action'),array($message));exit;
			}else{
				$debug && exit('__ERROR__::'.str_replace('\\','.',$base_error_class).'__'.$forward['action'].'Action__');
			}
		}	
	}
	/*******************
	...
	* @
	*/
	public static function getDI($name=null){return self::$_di;}
}
